...
async function loadTopCreators(tab) {
  ...
  try {
    if (tab === 'stars') {
      ...
      // Global prior used to "discount" low-sample-size ratings, so one lucky
      // 5★ from a single rater can't outrank a comic with dozens of ratings
      // that average slightly less. Same idea as IMDb's weighted rating.
      const ratedComicsList = Object.values(ratingsByComic).filter(r => r.count > 0);
      const globalMeanRating = ratedComicsList.length
        ? ratedComicsList.reduce((a, r) => a + r.sum, 0) / ratedComicsList.reduce((a, r) => a + r.count, 0)
        : 0;
      const globalAvgRaters = ratedComicsList.length
        ? ratedComicsList.reduce((a, r) => a + r.count, 0) / ratedComicsList.length
        : 1;
      const CONFIDENCE_M = Math.max(1, globalAvgRaters); // "how many raters counts as reliable", scales with app size

      function weightedScore(avg, count) {
        return (count / (count + CONFIDENCE_M)) * avg + (CONFIDENCE_M / (count + CONFIDENCE_M)) * globalMeanRating;
      }
