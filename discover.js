...
async function loadTopCreators(tab) {
  ...
  try {
    if (tab === 'stars') {
      // Rank by each creator's best-rated comic (highest avg rating).
      // This prevents prolific-but-low-rated creators from topping the board
      // simply by volume. A creator with one 4.8-rated comic ranks above one
      // whose many comics average 2.0.

      // Build a map of comicId → { sumRatings, count } from globalStars/ratings
      ...
      // For each owner, find their comic with the highest avg rating
      ...
      // Fall back to comic count for owners with no rated comics at all
      const comicsByOwner = {};
      allComics.forEach(c => { if (c.owner_handle) comicsByOwner[c.owner_handle] = (comicsByOwner[c.owner_handle] || 0) + 1; });

      // Merge: owners with rated comics rank above those without (avgRating 0)
...
function renderTopCreators(creators, tab) {
  ...
  list.innerHTML = creators.map((c, i) => {
    ...
    // Stars tab: gold star icon with rating inside + (n) rater count — same style as comic tiles
    // Follows tab: plain follower count with 👥
    let statHtml;
    if (tab === 'stars') {
